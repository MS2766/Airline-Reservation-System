...
if (isset($_POST['pass_but'])) {
    ...
    echo "Pass Form not submitted.";
}
/*use PHPMailer\PHPMailer\PHPMailer;*/
if(isset($_POST['pay_but']) && isset($_SESSION['user_id'])) {
    ...
    if(!mysqli_stmt_prepare($stmt,$sql)) {
        header('Location: 2.payment.php?error=sqlerror');
        exit();            
    } else {
        mysqli_stmt_bind_param($stmt,'isiis',$_SESSION['user_id'],
                ...
                if($row = mysqli_fetch_assoc($result)) {
                    ...
                    if(!mysqli_stmt_prepare($stmt,$sql)) {
                        ...
                        mysqli_stmt_bind_param($stmt,'iisisi',$i,
                            $flight_id,$new_seat,$price,$class,$_SESSION['user_id']);            
                        mysqli_stmt_execute($stmt);  
            ...
            for($i=$pass_id;$i<=$passengers+$pass_id;$i++) {
                ...
                if(!mysqli_stmt_prepare($stmt,$sql)) {
                    ...
                    if($row = mysqli_fetch_assoc($result)) {                        
                        ...
                        header('Location: 2.payment.php?error=noret');
                        exit();                     
                    }
                }   
            }             
        ...
        if($flag) {
            ...
            header('Location: 2.paysuccess.php');
            exit();    
